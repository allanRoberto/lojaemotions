...
class GFForms {
	...
	private static function can_upgrade_longtext( $previous_version ) {
		...
		if ( version_compare( $previous_version, '2.0-beta-3.2', '<' )  // No upgrades have been attempted.
		     || ( ! method_exists( $wpdb, 'get_col_length' ) )          // $wpdb->get_col_length() was introduced in WP 4.2.1. Attempts to upgrade will have caused a fatal error.
		     || ( version_compare( $previous_version, '2.0.2.5', '<' )  // Some upgrades prior to 2.0.2.5 failed because $wpdb->get_col_length() returned false. e.g. installations using HyperDB
	...
	private static function verify_lead_detail_longtext_upgrade_prerequisites() {
		...
		if ( $col_type == 252 ) {
			// TEXT col types, inc. LONGTEXT have type 252
			$is_longtext = true;
		}
